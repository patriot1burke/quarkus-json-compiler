...
public class ParserContext {
    ...
    public void rewind() {
        if (buildingToken) throw new RuntimeException("Cannot rewind while building token");
        ptr--;
        if (ptr < -1) throw new RuntimeException("Busted buffer bounds");
    }
    public void startToken() {
        if (buildingToken) throw new RuntimeException("Already building token");
        buildingToken = true;
        tokenStart = ptr - 1;
        if (tokenStart < 0) throw new RuntimeException("Unknown state");
    }
    ...
    public long popLongToken() {
        long val;
        if (tokenBuffer == null) {
            int start = tokenStart < 0 ? 0 : tokenStart;
            if (tokenEnd < 0) {
                throw new RuntimeException("Token not ended.");
            }
